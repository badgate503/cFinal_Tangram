...
string gameList[GAME_NUM] = {"方块","天鹅","房子","猫","树","乌龟","M","鸟","桥","三角形"};
string pathList[GAME_NUM] = {"square.tgm","swan.tgm","house.tgm","cat.tgm","tree.tgm","turtle.tgm","M.tgm","bird.tgm","bridge.tgm","triangle.tgm"};
	...
	else
	{
		...
		else
		{
			for(j = 6; j >= 0;j--)
			{
				...
				{
					isDragging = FALSE;
					//adjustPosition(shpList[6],vecList); 
				}
...
void ButtonEventProcess(int btnIndex)
{
	static bool isIllustrate=FALSE;
	switch(btnIndex){
    case 0  :
       	loadGame("");
       	break; 
	...
	case BTN_NUM+1:
		...
		break;
	case BTN_NUM+2:
		isIllustrate = isIllustrate?FALSE:TRUE;		//再次点击“关于”键，isIllustrate变为！isIllustrate
		displayIllustrate(isIllustrate);			//输出说明 
		break;
	case BTN_NUM+3:
		ExitGraphics(); 
		break;
	case BTN_NUM+4:
		...
		isIllustrate=FALSE;
		displayIllustrate(isIllustrate);
		break;
	case BTN_NUM+5:
